...
The challenge:
                ...
                An image is shown with white letters on a mixed colored background
'''
...
The writeup:
                ...
                Only details to consider is to download 2 recognition models by running this command:
                
                ' easyocr.Reader(['en','fr'], recog_network = 'latin_g1', gpu = False) '
                *note: we are using the older recog_network, it's seem to give us less false positive with the font used*

                You have to do this BEFORE every query of the page for the challenge, but only the first time it is downloading the language models.

                Then for the rest we don't need to preprocess the image at all, we just decode the base 64 and feed the raw bytes to easyOCR
                and VOILÀ!
